...
// Return active file metadata without exposing full source text.
function active_file_text(root, current_file, active_tab, open_texts)
  ...
  text_len = 0
  if typeof(open_texts) == "array" and active_tab >= 0 and active_tab < len(open_texts) then text_len = len(open_texts[active_tab]) end if
  return "active_file: " + relative_path(root, current_file) + "\nactive_text_length: " + text_len + "\n"
end function
...
function tool_context(project_value, compiler_path, config, current_file, open_files, open_dirty, active_tab, open_texts)
  ...
  if config.include_tabs then text = text + open_tabs_text(project_root, open_files, open_dirty, active_tab) + "\n" end if
