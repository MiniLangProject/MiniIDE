...
function _create_fixture(root)
  ...
  _mkdir(project_model.path_join(root, "tests"))

  fs.writeAllText(project_model.path_join(root, "ServiceTest.mlproj"),
    "name=ServiceTest\n" +
    "type=console\n" +
    "entry=src\\main.ml\n" +
  ...
  fs.writeAllText(project_model.path_join(root, "src\\util.ml"),
  ...
  fs.writeAllText(project_model.path_join(root, "tests\\main_test.ml"),
    "import \"..\\src\\main.ml\" as app\n" +
    "function test_model_lifecycle()\n" +
    "  return 0\n" +
    "end function\n")

  fs.writeAllText(project_model.path_join(root, "tests\\util_test.ml"),
...
function main(args)
  ok = true
  root = project_model.path_join("build", "ServiceTestProject")
  suffix = 0
  while fs.exists(root)
    suffix = suffix + 1
    root = project_model.path_join("build", "ServiceTestProject_" + suffix)
  end while
  ...
  broken_project = project_model.with_compile_settings(p, "src\\missing_entry.ml", p.output)
  broken_project = project_model.with_run_settings(broken_project, "tests\\missing_test.ml", p.run_args, "missing_workdir")
  ...
  fs.writeAllText(project_model.path_join(root, "BrokenImportPath.mlproj"),
  ...
  broken_import_project = project_model.load_project_file(project_model.path_join(root, "BrokenImportPath.mlproj"))
  ...
  fs.writeAllText(project_model.path_join(root, "src\\duplicate_alias.ml"),
    "import \"util.ml\" as dup\n" +
    "import \"main.ml\" as dup\n" +
    "function duplicateAliasFixture()\n" +
    "  return 0\n" +
    "end function\n")
  duplicate_project = project_model.load_project_file(project_model.path_join(root, "ServiceTest.mlproj"))
  ...
  related_tests = service.related_test_items(snapshot, project_model.path_join(p.root, "src\\main.ml"), 20)
  if _assert_true("related tests include importing test file", _has_test_item(related_tests, project_model.path_join(p.root, "tests\\main_test.ml"), "file", "related")) == false then ok = false end if
  ...
  related_file = service.related_test_file(snapshot, project_model.path_join(p.root, "src\\main.ml"))
  if _assert_true("related test file picks importing test file", related_file == project_model.path_join(p.root, "tests\\main_test.ml")) == false then ok = false end if
  convention_tests = service.related_test_items(snapshot, project_model.path_join(p.root, "src\\util.ml"), 20)
  if _assert_true("related tests include convention test file", _has_test_item(convention_tests, project_model.path_join(p.root, "tests\\util_test.ml"), "file", "related")) == false then ok = false end if
  ...
  convention_file = service.related_test_file(snapshot, project_model.path_join(p.root, "src\\util.ml"))
  if _assert_true("related test file picks convention test file", convention_file == project_model.path_join(p.root, "tests\\util_test.ml")) == false then ok = false end if
  ...
  main_path = project_model.path_join(p.root, "src\\main.ml")
