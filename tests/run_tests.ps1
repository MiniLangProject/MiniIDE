...
function Test-StaticWiring {
  ...
  Assert-True ($main.Contains("if hwnd == enabled_btn then enabled = enabled == false end if")) "Assistant settings mouse clicks must toggle the assistant enabled button."
  Assert-True ($main.Contains("Dispatch the button release first")) "Assistant settings must dispatch button releases before applying actions."
  ...
  Assert-True ($main.Contains("ctrl and win.msg_hwnd(msg) == st.editor and key == win.VK_A")) "Ctrl+A Select All editor hotkey is missing."
  Assert-True ($main.Contains("ctrl and win.msg_hwnd(msg) == st.editor and key == win.VK_V")) "Ctrl+V editor paste hotkey is missing."
  Assert-True (-not $main.Contains("if ctrl and _key_pressed(st, win.VK_V) then return _edit_paste(st) end if")) "Ctrl+V must not also be handled by polling hotkeys."
